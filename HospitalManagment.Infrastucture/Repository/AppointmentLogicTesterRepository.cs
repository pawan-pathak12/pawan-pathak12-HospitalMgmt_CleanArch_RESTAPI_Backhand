...
namespace HospitalManagment.Infrastucture.Repository
{
    public class AppointmentLogicTesterRepository : IAppointmentLogicTester
    {
        ...
        {
            using var connection = _dapperDbContext.Connection();
            string query = "select count(*) from Appointments where DoctorId=@DoctorId and " +
                " AppointmentDate= @AppointmentDate";
            var result = await connection.QuerySingleAsync<int>(query, new { DoctorId = doctorId , AppointmentDate = DateTime.Now  });
            return result;
        }
        ...
        {
            using var connection = _dapperDbContext.Connection();
            var query = "select count(1) from Appointment where PatientId =@PatientId and (Status=@Status1 or Status=@Status2)";
            var result = await connection.ExecuteAsync(query, new { PatientId = patientId , Status1 = AppointmentStatus.Cancelled , Status2= AppointmentStatus.NotShown });
            return result ;
        }
